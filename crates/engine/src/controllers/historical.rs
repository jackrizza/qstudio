...
impl<'a> HistoricalController<'a> {
    ...
    pub fn execute(&self) -> Result<DataFrame, String> {
        // Implement the logic to handle historical queries
        log::info!("Executing HistoricalController for frame: {:?}", self.frame);

        let data: Arc<Mutex<Option<DataFrame>>> = Arc::new(Mutex::new(None));

        let rt: tokio::runtime::Runtime = tokio::runtime::Runtime::new().unwrap();
        let output: Result<DataFrame, String> =
            rt.block_on(pull_data(&self.frame.model));

        let df = match output {
            Ok(df) => {
                log::info!("Data fetched successfully with {} rows", df.height());
                df
            }
            Err(e) => {
